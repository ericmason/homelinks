...
/* Titles a page never meant to be known by. A site behind a bot check answers
   the first request with "Just a moment..." and the browser files that as the
   page's title, so a link added to stackoverflow.com comes out named after
   Cloudflare's waiting room. Rare on a page you sat and read, common on one you
   passed through, and always wrong as a name. */
const JUNK = /^(just a moment|one moment|please wait|checking your browser|attention required|access denied|verify you are human|redirecting|loading|untitled|error|\d{3}\b)/i;
const usable = (t) => !!(t || '').trim() && !JUNK.test(t.trim());
...
/* A name for a link the user just pasted. The browser already holds a title for
   ...
   -- "Hacker News" rather than news.ycombinator.com. The exact page first,
...
export async function titleFor(url) {
  ...
  const mine = hits.filter(v => v.url && usable(v.title) && hostOf(v.url) === h);
...
/* Nothing in history, so the tidiest thing the URL itself offers: the domain,
   ...
   nobody calls the site by. acme.atlassian.net -> Atlassian. */
...
export async function topSites({ limit = 40, minVisits = 3, pagesPerHost = 0, days = 180 } = {}) {
  ...
  for (const it of items) {
    ...
    if (vc > a._top && usable(it.title)) { a._top = vc; a.title = cleanTitle(it.title, h); }
  }
