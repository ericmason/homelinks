...
- A new tab opens finished. The links, their favicons, the Frequent row, and your background photograph are all in the first frame instead of arriving over the next fifth of a second. Links reach the screen in 20 ms rather than 160 ms on a profile with six months of history. (PRPR)
- Favicons are remembered between tabs. Working out which icon a link wears took up to three fetches per link, on every tab; now it is done once, checked again a day later, and never in front of a paint. A link the browser has no icon for is rechecked within the hour, so an icon appears soon after you first visit the site. (PRPR)
- A background photograph appears immediately rather than after a moment of gradient. Each image gets a 160-pixel copy that the page opens on while the full-size one is read and decoded behind it, then swapped in. It is the same picture, so nothing moves — it only sharpens. (PRPR)
- The Frequent row shows the last count while a new one is taken behind it. Counting means reading six months of this profile's history, which was the single slowest thing between opening a tab and using it. Counts more than six hours old are retaken after the page is up, and **recount** in ⌘B still counts on the spot. (PRPR)
- The extension keeps two more things in local storage: which icon each of your links wears, and a thumbnail and name per background image. Both are derived from what it already had, neither leaves the device, and removing a link or an image removes its entry. (PRPR)
- The page fades in over 0.2 s instead of 0.42 s. The content was already complete when the old animation started, so the animation was the wait. (PRPR)
